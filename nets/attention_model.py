...
def set_decode_type(model, decode_type):
    """
    为模型设置解码类型，主要用于解码时选择节点的方式
    :param model:
    :param decode_type: "greedy"或"sampling"
    :return:
    """
...
class AttentionModel(nn.Module):
    ...
    def forward(self, input, return_pi=False):
        ...
        # pi: (batch, graph_size)
        _log_p, pi = self._inner(input, embeddings)  # 得到所有解码步的log_p
        # 计算当前图在pi上的路径长度
        # cost: (batch,)  mask=None
    ...
    def _inner(self, input, embeddings):
        ...
        state = self.problem.make_state(input)   # 初始化相关信息

        # Compute keys, values for the glimpse and keys for the logits once as they can be reused in every step
        fixed = self._precompute(embeddings)   # 计算固定的context

        batch_size = state.ids.size(0)   # 获取batch_size

        # Perform decoding steps
        i = 0
        while not (self.shrink_size is None and state.all_finished()):  # 同时为真时退出
            ...
            # log_p: 当前步选择各节点的概率 (batch, 1, graph_size)
            # mask: 掩码张量 (batch, 1, )
            log_p, mask = self._get_log_p(fixed, state)    # 根据当前嵌入和状态计算输出

            # Select the indices of the next nodes in the sequences, result (batch_size) long
            # 根据log_p和mask，按照设置的解码类型(greedy或sampling)选择节点 (batch,)
            selected = self._select_node(log_p.exp()[:, 0, :], mask[:, 0, :])  # Squeeze out steps dimension
            # 更新状态
            # prev_a(上一步动作)，cur_coord(当前坐标)，lengths(路径长度)，
            # first_a(第一步动作，如果是第一次解码要更新)，visited_(已访问节点)(依据这个在下次循环中mask也会得到更新)
        ...
        # forward函数中的_log_p: (batch_size, num_steps, graph_size)
        # pi: (batch, num_steps)
    ...
    def _select_node(self, probs, mask):
        """
            :param probs: 选择各节点的概率 Tensor (batch, lenth)，已经进行过掩码处理，不能选择的节点选择的概率应该是0
            :param mask: mask张量 Tensor (batch, lenth)，需要掩码的节点在该张量中设置为1
            :return: 当前时间步选择输出的节点 Tensor (batch,)
        """
    ...
    def _precompute(self, embeddings, num_steps=1):
        ...
        # 把上面的数据放到AttentionModelFixed中并返回
